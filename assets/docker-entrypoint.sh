...
function waitForCockroachStartup() {
  echo
...
function migrateDatabase() {
  ...
  for SCRIPT in /docker-entrypoint-initdb.d/*; do
    ...
    if [[ -f ${STAT_FILE} ]]; then
      ...
      echo -e "\nRunning ${SCRIPT}..."
      /cockroach/cockroach sql ${COCKROACH_SECURITY_OPTS} --database ${COCKROACH_DATABASE:-defaultdb} --user ${COCKROACH_USER:-root} <${SCRIPT}
      touch ${STAT_FILE}
    else
      echo -e "\nSkipping ${SCRIPT}."
    fi
...
function waitForCockroachShuttingDown() {
